...
- [x] Engine auto start after reset, even though state == stopped (fixed: `Engine.resetEngine()` was deciding "was running" from `moduleManager.size > 0`, but `stopAll()` leaves stopped instances in the map — so it stayed truthy after a manual stop. Engine now tracks an explicit `_modulesRunning` flag flipped only by `startModules`/`stopModules`, and reset reads that.)
- [x] When i clone / add a module it auto starts event though engine state == stopped (Only when cloning from the right click menu) (fixed: `EnginePatchRouter` unconditionally called `lifecycle.startSingle` on every `/modules/<id>` add side effect. It now consults the new `getModulesRunning` getter and skips the auto-start when the engine is stopped — clone and add-from-panel both honour engine state.)
- [x] If VU meter packet goes missing (Does not reach the manager) don't immidiatly show vu as 0, but rather have a timeout of say 2 seconds, otherwise it looks like there is a audio issue, but was rather just a packet that did not make it on the vu data (fixed: bumped `STALE_MS` in `vuMeters.ts` from 1500 to 2000 — last value is now held for 2s before zeroing, which covers ~30 missed VU cycles at the engine's 15Hz cadence and tolerates the 1s heartbeat too. Test rewritten to assert both ends of the boundary.)
- [x] Clonging a module from right click menu works fine, cldonign a module from settings, the module is blank, and have to refresh for it to show correctly, how is that? is it not using the same logic to clone ? (fixed: two duplicate clone implementations — right-click sent the full module shape, settings panel only sent pluginId/displayName/position/settings, so the optimistic local store had no ports/schema/color/icon and rendered blank. Extracted `patch.cloneModule(engineId, moduleId)` as the single source of truth; both call sites now use it.)

- [x] !!!! When srt fails to connect, device cpu spikes quite heavely
- [x] !!! engine ID is used for authentication, can we make the engineID editable? (fixed: added `EngineRepository.rename` + `ConfigStore.renameEngine` that re-PK the engine and follow the FK through `engine_profiles`/`engine_config_history` inside a `defer_foreign_keys` transaction, since better-sqlite3 enforces FKs by default; the `engine:update` Socket.IO RPC accepts a `newEngineId` to rename, refreshes the dgram-comms encryption-key map, and emits `engine:renamed` so the UI rekeys its engine Map without losing insertion order. UI exposes the field in the Edit modal with an explicit warning that the engine itself authenticates from its local `profiles.json` and won't reconnect until the operator updates `profile.name` on the engine host. Manager-side rename only — coordinating the engine-side profile update is left to the operator.)
- [x] After changeing profiles, i need to restart the engine, other wise it does not start, (fixed: `profile:activate` only pushed the new config to the engine — modules from the previous profile kept running because nothing triggered a restart. Manager now follows the config push with a `start` command when `engineCommands.isRunning(engineId)` is true; the engine's `lifecycle.startAll()` destroys the old modules and creates the new ones from the just-pushed config in a single serialized lifecycle step. When the engine is stopped only the config is pushed, so toggling profiles no longer accidentally starts modules.)
- [x] Build a reboot command into the media-router manager dashboard (8080) (done: new `Reboot host` entry in the engine sidebar context menu fires `engine:reboot` over Socket.IO; `SocketIOSetup` forwards a `{ command: 'reboot' }` dgram-comms message to the online engine; `CommandDispatcher` handles `'reboot'` by calling `Engine.rebootHost()` which execs `systemctl reboot` (fire-and-forget — the host goes down with the engine). Confirmation modal in `AppSidebar` prevents accidental triggers. Needs a polkit rule on the target image allowing `org.freedesktop.login1.reboot` for the engine user. Failures (typically polkit denial) round-trip back via a new `rebootFailed` engine topic → `engineRebootFailed` on `EngineConnectionManager` → `engine:rebootFailed` Socket.IO broadcast → `useSocketStore.rebootFailure` ref → `AppSidebar` failure modal, so the operator sees the reason instead of a silent non-reboot. 1162 tests passing.)