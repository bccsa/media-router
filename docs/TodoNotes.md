...
- [ ] If VU meter packet goes missing (Does not reach the manager) don't immidiatly show vu as 0, but rather have a timeout of say 2 seconds, otherwise it looks like there is a audio issue, but was rather just a packet that did not make it on the vu data
- [ ] Clonging a module from right click menu works fine, cldonign a module from settings, the module is blank, and have to refresh for it to show correctly, how is that? is it not using the same logic to clone ?  
