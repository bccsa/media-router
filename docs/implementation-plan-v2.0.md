...
| 1.2 | Fragmentation module | Split messages >1412 bytes into numbered chunks. Each fragment: `[messageId: 4B][fragmentIndex: 2B][fragmentCount: 2B][payload]` (`fragmentCount === 0` = NACK control packet). Reassembly keyed by `source:messageId` with timeout + fragment-level NACK retransmit. `fragment(data) → Buffer[]`, `fragmentWithId(data)`, `FragmentTransport` class | UR-COM-005 | §8.1.1–8.1.2 |
