...
| [0016](0016-subtitle-carrier-klv-webvtt.md) | Subtitles travel as KLV-wrapped WebVTT cues (`meta/x-klv`, house-clock times) via `subtitle-core` (its python bridge rides the generic `runnerHooks` seam); renderers set `textoverlay`'s `text` from a video-pad probe, never the text pad |
