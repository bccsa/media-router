...
| [0017](0017-media-agnostic-muxer-inputs.md) | Muxer inputs are media-agnostic: N generic `input-N` ports, every demuxed stream routed by class (video/audio/klv/subtitle) into its input's 8-PID block (`muxInputClassPid(muxInputBasePid(N), class)`; legacy `video-N`/`audio-N` ports keep the per-kind `muxSlotPid` ranges) by the plugin's own runner hook `mux_routing.py` (unrouted pads → fakesink, PCR picked video-first; no engine code); one operator PID per input (8-PID block, written back when automatic); legacy `video-N`/`audio-N` configs keep their ports and PIDs; the D2 name carousel is retired — identity = ISO 639 descriptors only |
...
| [0019](0019-native-pipeline-runner.md) | A native C++ runner (`packages/engine/native/mr-gst-runner`, same JSON protocol) hosts every pipeline whose description stays inside its feature set when `MR_GST_RUNNER_NATIVE=1` (or `runner: 'native'`); python-only features (legacy librist stats, KLV payload, `preserveSourceTimeline`, stdio data, legacy net clock, hooks without a native form) keep the python runner; the runner refuses what the engine's eligibility list excludes so the two cannot drift |
