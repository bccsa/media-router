...
| [0023](0023-one-session-many-paths.md) | One dgram-comms session per engine spans every network path: the manager listens on 1 + n UDP ports (`manager_settings.dgramListeners`, UI-edited, live rebind), a same-nonce `connect` from a new endpoint JOINS the session, both sides fan out to every live endpoint with one shared seq and dedup on (session, seq), a silent endpoint is pruned (`pathDown`) while the session lives on; deploy the manager first |
