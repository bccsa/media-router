...
| D3 | **Deterministic PIDs** via mpegtsmux request-pad naming (`sink_<pid>`): video-N → 0x100+N, audio-N → 0x140+N, subtitle headroom above, metadata PID fixed. *Generalised by ADR-0017 (2026-09-15):* muxer inputs are media-agnostic, so a GENERIC input N owns the 8-PID block 0x100 + 8·N — video +0, audio +1, klv +2, subtitle +3 (`muxInputClassPid`, plugin-local `muxPids.ts`; one operator `pid` per input overrides the block; routing = the plugin's `mux_routing` runner hook); legacy `video-N`/`audio-N` ports keep the original per-kind connected-ordinal PIDs (`muxSlotPid`: video 0x100+, audio 0x140+, klv 0x180+, subtitle 0x1a0+). 0x1f0 stays reserved (older muxers' carousel PID, dropped downstream). | PID is the stable join key between detection and naming, and makes demuxer port identity stable across restarts. |
