...
| D2 | **Name channel = private KLV metadata PID** (`meta/x-klv`), JSON payload, re-sent ~1/s carousel-style. *RETIRED 2026-09-16 (ADR-0017): the muxer no longer emits it — the reader went with the mpegts-demuxer and the ts-splitter labels from PMT descriptors (ISO 639) only.* | Single-program TS preserved (tsdemux only demuxes one program at a time, so SDT-per-stream would force `tee ! N× tsdemux`). mpegtsmux/tsdemux KLV caps confirmed on fleet GStreamer 1.22. Invisible-but-harmless to third-party receivers. |
| D3 | **Deterministic PIDs** via mpegtsmux request-pad naming (`sink_<pid>`): video-N → 0x100+N, audio-N → 0x140+N, subtitle headroom above, metadata PID fixed. *Generalised by ADR-0017 (2026-09-15):* muxer inputs are media-agnostic, so a GENERIC input has ONE `pid` — the output PID of its stream (next free from 0x100 in steps of 8 when automatic, written back; *amended 2026-09-18*, the 8-PID block of the first cut is gone); a multi-stream source puts its highest class on that PID and the rest on the next free PIDs, decided by the plugin's `mux_routing` runner hook from the source PMT; legacy `video-N`/`audio-N` ports keep the original per-kind connected-ordinal PIDs (`muxSlotPid`: video 0x100+, audio 0x140+, klv 0x180+, subtitle 0x1a0+). 0x1f0 stays reserved (older muxers' carousel PID, dropped downstream). | PID is the stable join key between detection and naming, and makes demuxer port identity stable across restarts. |
...
| D5 | **Discovery populates config; it never replaces it.** Discovered streams are written into the demuxer module config; ports persist offline and missing streams show as stale (red node badge + panel entries). *Refined post-Phase 3:* a stale stream with **zero connected consumers** is garbage-collected after the post-start grace; anything with a connection is never auto-removed. *Implemented for the ts-splitter 2026-09-18 (ADR-0021):* GC happens on the discovery event that omits the PID (so only a live source with a different PMT prunes), "connected" means a STORED connection (consumer active or not), no grace. | Preserves the configure-then-run broadcast model — ports and downstream connections must survive a source going dark. Unconnected-port GC is risk-free: re-discovery recreates the identical PID-derived port id. |
...
- Demuxer: discovery writes found streams into module config (D5) as a
  ...
  and rendered **stale** while a stored connection references it; otherwise it
  is pruned on the next discovery (ADR-0021, ts-splitter). The metadata PID is never
