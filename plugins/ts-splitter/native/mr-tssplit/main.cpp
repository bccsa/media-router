...
//                   [--stall-ms 2000] [--flush-ms 20] [--stamp-timeline]
//                   [--no-latch-repair] --out 0x100:busout_40001[:0x1b] ...
#include <fcntl.h>
#include <poll.h>
#include <signal.h>
#include <unistd.h>
...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        } else if (a == "--stamp-timeline") {
            opts.stamp_timeline = true;
        } else if (a == "--no-latch-repair") {
            opts.repair_latch = false;
    ...
    if (opts.input_socket.empty() || opts.caps.empty()) {
        std::fprintf(stderr,
                     ...
                     "[--ts-id N] [--stall-ms N] [--flush-ms N] [--stamp-timeline] "
                     "[--no-latch-repair] --out pid:tee[:stype] ...\n");
        return 2;
    }
