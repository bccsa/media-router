...
- `native/mrrist/` — GStreamer plugin with `mrristsrc` / `mrristsink`, librist
  ...
  `mrrist-stats` bus messages carrying librist's JSON; the receiver also posts
  `mrrist-peer` `{id, cname}` (the sender's SDES name per peer id, lifted from
  librist's log — the receiver stats JSON names peers by id only).
