...
const GOLDEN_V4L2_SCALER =
    'unixfdsrc socket-path=/tmp/mr-bus-5000.sock ! queue leaky=2 max-size-time=5000000000 max-size-buffers=0 max-size-bytes=40000000 ! queue leaky=0 max-size-time=700000000 max-size-buffers=0 max-size-bytes=0 ! tsparse set-timestamps=false ! tsdemux name=demux latency=0 ! capsfilter caps="video/x-h264" ! h264parse ! avdec_h264 thread-type=frame max-threads=3 ! queue leaky=2 max-size-time=700000000 max-size-buffers=0 max-size-bytes=0 ! videorate drop-only=true ! video/x-raw,framerate=25/1 ! tee name=t t. ! queue leaky=2 max-size-buffers=4 max-size-time=0 max-size-bytes=0 ! v4l2convert ! video/x-raw,width=1920,height=1080 ! v4l2h264enc name=venc_0 extra-controls="controls,video_bitrate=5000000,video_bitrate_mode=0,repeat_sequence_header=1,video_gop_size=60,h264_i_frame_period=60" ! video/x-h264,level=(string)4.2 ! h264parse config-interval=1 ! mpegtsmux name=mux_0 latency=0 alignment=0 ! capssetter caps="video/mpegts, systemstream=(boolean)true, packetsize=(int)188" replace=true ! capsfilter caps="video/mpegts, systemstream=(boolean)true, packetsize=(int)188" ! tee name=busout_40100 allow-not-linked=true t. ! queue leaky=2 max-size-buffers=4 max-size-time=0 max-size-bytes=0 ! videoscale ! video/x-raw,width=854,height=480 ! videoconvert ! x264enc name=venc_1 tune=zerolatency bitrate=1200 speed-preset=medium key-int-max=60 bframes=0 interlaced=true option-string="vbv-maxrate=1800:vbv-bufsize=900:scenecut=30" ! video/x-h264,profile=baseline ! h264parse config-interval=1 ! mpegtsmux name=mux_1 latency=0 alignment=0 ! capssetter caps="video/mpegts, systemstream=(boolean)true, packetsize=(int)188" replace=true ! capsfilter caps="video/mpegts, systemstream=(boolean)true, packetsize=(int)188" ! tee name=busout_40101 allow-not-linked=true';

const GOLDEN_VA_SCALER =
    'unixfdsrc socket-path=/tmp/mr-bus-5000.sock ! queue leaky=2 max-size-time=5000000000 max-size-buffers=0 max-size-bytes=40000000 ! queue leaky=0 max-size-time=700000000 max-size-buffers=0 max-size-bytes=0 ! tsparse set-timestamps=false ! tsdemux name=demux latency=0 ! capsfilter caps="video/x-h264" ! h264parse ! avdec_h264 thread-type=frame max-threads=3 ! queue leaky=2 max-size-time=700000000 max-size-buffers=0 max-size-bytes=0 ! videorate drop-only=true ! video/x-raw,framerate=25/1 ! tee name=t t. ! queue leaky=2 max-size-buffers=4 max-size-time=0 max-size-bytes=0 ! videoscale ! video/x-raw,width=1920,height=1080 ! videoconvert ! v4l2h264enc name=venc_0 extra-controls="controls,video_bitrate=5000000,video_bitrate_mode=0,repeat_sequence_header=1,video_gop_size=60,h264_i_frame_period=60" ! video/x-h264,level=(string)4.2 ! h264parse config-interval=1 ! mpegtsmux name=mux_0 latency=0 alignment=0 ! capssetter caps="video/mpegts, systemstream=(boolean)true, packetsize=(int)188" replace=true ! capsfilter caps="video/mpegts, systemstream=(boolean)true, packetsize=(int)188" ! tee name=busout_40100 allow-not-linked=true t. ! queue leaky=2 max-size-buffers=4 max-size-time=0 max-size-bytes=0 ! videoscale ! video/x-raw,width=854,height=480 ! videoconvert ! x264enc name=venc_1 tune=zerolatency bitrate=1200 speed-preset=medium key-int-max=60 bframes=0 interlaced=true option-string="vbv-maxrate=1800:vbv-bufsize=900:scenecut=30" ! video/x-h264,profile=baseline ! h264parse config-interval=1 ! mpegtsmux name=mux_1 latency=0 alignment=0 ! capssetter caps="video/mpegts, systemstream=(boolean)true, packetsize=(int)188" replace=true ! capsfilter caps="video/mpegts, systemstream=(boolean)true, packetsize=(int)188" ! tee name=busout_40101 allow-not-linked=true';
