...
class _paNullSinkSourceBase extends _paAudioSourceBase {
    ...
    Init() {
        ...
        this._parent.on('sinks', sinks => {
            if (sinks.find(t => t.name == this.sink) && sinks.find(t => t.name == this._sink) ) {
                setTimeout(() => {
                    this.ready = true;
                },100)
            } else {
    ...
    _startNullSink() {
        ...
        exec(cmd_sink, { silent: true }).then(data => {
            if (data.stderr) {
            ...
            if (data.stdout.length) {
                ...
                this._parent._log('INFO', `${this._controlName} (${this.displayName}): Created null-sink; ID: ${this._paSinkModuleID}`);
            }
        }).catch(err => {
        ...
        exec(cmd_source, { silent: true }).then(data => {
            if (data.stderr) {
                this._parent._log('ERROR', data.stderr.toString());
            }
            ...
            if (data.stdout.length) {
                ...
                this._parent._log('INFO', `${this._controlName} (${this.displayName}): Created null-sink; ID: ${this._paSourceModuleID}`);
            }
        }).catch(err => {
            this._parent._log('FATAL', `${this._controlName} (${this.displayName}) - Unable to start null-sink: ` + err.message);
        });
