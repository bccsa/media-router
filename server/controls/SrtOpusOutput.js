    ...
    _start_gst() {
        if (!this._gst) {
            try {
                ...
                let args = `pulsesrc device="${this.source}" latency-time=${this._parent.paLatency * 1000} ! audio/x-raw,rate=${this.sampleRate},format=S${this.bitDepth}LE,channels=${this.channels} ! audioconvert ! audioresample ! queue leaky="upstream" max-size-bytes=0 ! opusenc bitrate=${this.bitrate * 1000} audio-type="restricted-lowdelay" ! mpegtsmux alignment=7 latency=1 ! srtsink wait-for-connection=false uri="srt://${this.srtHost}:${this.srtPort}?mode=${this.srtMode}&pkt_size=188&latency=${this.srtLatency}${streamID}${crypto}"`;
