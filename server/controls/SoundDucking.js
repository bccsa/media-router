...
class SoundDucking extends _paNullSinkBase {
    ...
    _startAudioDucker() {
        if (this.side_chain) {
            ...
            const _pl = `pulsesrc device=${this.side_chain} ! audio/x-raw,rate=${this.sampleRate},format=S${this.bitDepth}LE,channels=1 ! audioconvert mix-matrix="<<(float)1.0>>" ! queue ! level peak-falloff=120 peak-ttl=50000000 interval=${this.vuInterval * 1000000} ! fakesink silent=true`;
