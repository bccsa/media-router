...
class SoundDucking extends _paNullSinkBase {
    ...
    _startAudioDucker() {
        if (this.side_chain) {
            this._parent._log(
                "INFO",
                `${this._paModuleName} (${this.displayName}): Starting Audio Ducker`
            );
            const _pl = `pulsesrc device=${
                ...
                this._srcChannels
            } ! deinterleave name=d audiomixer name=i ! level peak-falloff=120 peak-ttl=50000000 interval=${
                this.vuInterval * 1000000
            } ! fakesink silent=true ${this._channelMap}`;
            ...
            this._parent._log(
                "FATAL",
                `${this._paModuleName} (${this.displayName}): Unable to start Audio Ducker, Side chain device is not available`
            );
        }
    }
    ...
    _stopAudioDucker() {
        this._parent._log(
            "INFO",
            `${this._paModuleName} (${this.displayName}): Stopping Audio Ducker`
        );
        vu.stop_vu();
    }
