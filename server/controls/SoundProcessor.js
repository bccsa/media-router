...
class SoundProcessor extends Classes(_paNullSinkSourceBase, GstBase) {
    ...
    Init() {
        ...
        for (let i = 0; i < 10; i++) {
            this.on(`band${i}`, (val) => {
                if (this.eq) this.set_gst("eq", "gdouble", `band${i}`, val);
            });
        }
    ...
    startPipeline() {
        if (this.ready && this.run) {
            // Source
            let _pipeline =
                `pulsesrc device=${this._source} latency-time=${
                    this._parent.paLatency * 1000
                } buffer-time=${this._parent.paLatency * 1000} ! ` + // do-timestamp=false used for live audio (see: https://github.com/ros-drivers/audio_common/issues/120)
                `audio/x-raw,rate=${this.sampleRate},format=S${
                    this.bitDepth
                }LE,channels=${this.channels},channel-mask=(bitmask)0x${(
                    Math.pow(2, this.channels) - 1
                ).toString(16)} ! audioconvert ! audiorate ! `;
            // EQ
            _pipeline += `equalizer-10bands name="eq" ${this._bands()} ! `;
            // Gate
            _pipeline += `calf-sourceforge-net-plugins-Gate name="gate" bypass=${!this
                .gate} knee=${this.gate_knee} ratio=${
                this.gate_ratio
            } threshold=${this.gate_threshold} attack=${
                this.gate_attack
            } release=${this.gate_release} makeup=${this.gate_makeup} ! `;
            // Compressor
            _pipeline += `calf-sourceforge-net-plugins-Compressor name="compressor" bypass=${!this
                .compressor} knee=${this.comp_knee} ratio=${
                this.comp_ratio
            } threshold=${this.comp_threshold} makeup=${this.comp_makeup} mix=${
                this.comp_mix
            } attack=${this.comp_attack} release=${this.comp_release} ! `;
            // Delay
            if (this.delay)
                _pipeline += `queue name="delay" leaky=2 min-threshold-time=${
                    this.delayVal * 1000000
                } max-size-buffers=0 max-size-bytes=0 max-size-time=${
                    (this.delayVal + 100) * 1000000
                } ! `;

            // Sink
            _pipeline += `pulsesink device=${
                this._sink
            } sync=false buffer-time=${
                this._parent.paLatency * 1000
    ...
    _bands(bandCount = 10) {
        ...
        for (let i = 0; i < bandCount; i++) {
            if (!this.eq) bands += `band${i}=0 `;
            else bands += `band${i}=${this[`band${i}`]} `;
        }
    ...
    _setBands(bandCount = 10) {
        for (let i = 0; i < bandCount; i++) {
            if (!this.eq) this.set_gst("eq", "gdouble", `band${i}`, 0);
            else this.set_gst("eq", "gdouble", `band${i}`, this[`band${i}`]);
        }
    }
}
