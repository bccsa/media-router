...
class SrtVideoEncoder extends Classes(_paNullSinkBase, SrtBase) {
    ...
    startPipeline() {
        if (this.ready && this.run) {
            ...
            let gstDeinterlace = "";
            if (this.deinterlace) {
                gstDeinterlace = `deinterlace mode=1 locking=2 ! `;
            }
            ...
            // video device
            let _pipeline =
                `v4l2src device=${this.video_device} do-timestamp=true ! ` +
                // capture format
                gstVideoDecode +
                // deinterlace
                gstDeinterlace +
                ...
                `videoscale n-threads=4 ! video/x-raw,width=${Math.ceil(
                    (this.video_quality / 9) * 16
                )},height=${this.video_quality} ! ` +
                // h264 encoding
                gstEncoder +
                // audio device
    ...
    _getV4l() {
        ...
        return new Promise((resolve, reject) => {
            exec("v4l2-ctl --list-devices || true", { silent: true })
                .then((data) => {
                    if (data.stderr)
                        this._parent._log(
                            "ERROR",
                    ...
                    if (data.stdout.length) {
                        ...
                        inputBlocks.map(function (block) {
                            arr.push(parseBlocks(block));
                        });
                    }
                    // remove invalid elements from arr
                    arr = arr.filter((r) => {
                        if (r.name != "" && r.name.search("platform") == -1) {
                            return r;
                        }
                    });
                    resolve(arr);
