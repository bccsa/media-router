...
async function createProfile() {
    ...
    try {
        ...
        if (!res.ok) { error.value = 'Failed to create profile'; return; }
...
async function switchProfile(name: string) {
    error.value = '';
    try {
        const res = await fetch(`/api/v1/engines/${props.engineId}/profiles/${name}/activate`, { method: 'POST' });
        if (!res.ok) { error.value = 'Failed to activate profile'; return; }
...
async function deleteProfile(name: string) {
    error.value = '';
    try {
        const res = await fetch(`/api/v1/engines/${props.engineId}/profiles/${name}`, { method: 'DELETE' });
        if (!res.ok) { error.value = 'Failed to delete profile'; return; }
...
async function loadHistory(profileName: string) {
    if (showHistory.value === profileName) { showHistory.value = null; return; }
    showHistory.value = profileName;
    try {
        const res = await fetch(`/api/v1/engines/${props.engineId}/profiles/${profileName}/history`);
...
async function rollback(versionId: number) {
    ...
    error.value = '';
    try {
        const res = await fetch(`/api/v1/engines/${props.engineId}/profiles/${showHistory.value}/rollback`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ versionId }),
        });
        if (!res.ok) { error.value = 'Failed to rollback'; return; }
...
async function exportProfile(name: string) {
    ...
    try {
        ...
        if (!res.ok) { error.value = 'Failed to export profile'; return; }
...
async function importProfile() {
    ...
    try {
        ...
        if (!res.ok) { error.value = 'Failed to import profile'; return; }
...
<template>
    <div class="p-6 max-w-2xl">
        ...
        <!-- Error banner -->
        <div v-if="error" class="mb-4 px-4 py-2 rounded-lg text-sm flex items-center justify-between"
             :style="{ backgroundColor: 'rgba(248, 113, 113, 0.15)', color: '#f87171', border: '1px solid rgba(248, 113, 113, 0.3)' }">
            <span>{{ error }}</span>
            <button @click="error = ''" class="ml-2 text-xs opacity-70 hover:opacity-100">Dismiss</button>
        </div>
        ...
        <div class="space-y-2">
            ...
            <div class="flex items-center justify-between px-4 py-3 rounded-lg"
                ...
                <div class="flex items-center gap-1.5">
                    <MrButton size="sm" variant="secondary" @click="loadHistory(p.profile_name)">
                        History
                    </MrButton>
                    <MrButton size="sm" variant="secondary" @click="exportProfile(p.profile_name)">
                        Export
                    </MrButton>
                    <MrButton v-if="engine?.activeProfile !== p.profile_name" size="sm" variant="secondary"
                              @click="showSwitchConfirm = p.profile_name">
                        Activate
                    </MrButton>
                    <MrButton v-if="engine?.activeProfile !== p.profile_name" size="sm" variant="danger"
    ...
    <MrModal v-if="showCreate" title="New Profile" @close="showCreate = false">
        <MrInput v-model="newProfileName" label="Profile Name" placeholder="e.g. Production"
    ...
    <MrModal v-if="showImport" title="Import Profile" @close="showImport = false">
        ...
        <template #footer>
            ...
            <MrButton @click="importProfile" :disabled="!importName.trim() || !importData.trim()">Import</MrButton>
        </template>
    </MrModal>
</template>
