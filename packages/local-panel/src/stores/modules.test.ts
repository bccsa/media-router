...
describe('LCP Module Store', () => {
    ...
    it('setAll populates modules', () => {
        ...
        store.setAll({
            'mic-1': { pluginId: 'audio-input', displayName: 'Mic 1', health: 'ok', running: true, ready: true, settings: { lcpVisible: true }, lcpType: 'mixer-strip' },
            'enc-1': { pluginId: 'audio-encoder', displayName: 'Encoder 1', health: 'ok', running: true, ready: true, settings: {}, lcpType: 'mixer-strip' },
        });
    ...
    it('visibleModules filters by lcpVisible and lcpType', () => {
        ...
        store.setAll({
            'mic-1': { pluginId: 'audio-input', displayName: 'Mic 1', health: 'ok', running: true, ready: true, settings: { lcpVisible: true }, lcpType: 'mixer-strip' },
            'srt-1': { pluginId: 'srt-input', displayName: 'SRT In', health: 'ok', running: true, ready: true, settings: {} }, // no lcpType
            'enc-1': { pluginId: 'audio-encoder', displayName: 'Encoder', health: 'ok', running: true, ready: true, settings: { lcpVisible: false }, lcpType: 'mixer-strip' },
        });
    ...
    it('visibleModules sorts by lcpSortOrder', () => {
        const store = useModuleStore();
        store.setAll({
            'out-1': { pluginId: 'audio-output', displayName: 'Output', health: 'ok', running: true, ready: true, settings: { lcpSortOrder: 10 }, lcpType: 'mixer-strip' },
            'mic-1': { pluginId: 'audio-input', displayName: 'Mic', health: 'ok', running: true, ready: true, settings: { lcpSortOrder: 1 }, lcpType: 'mixer-strip' },
            'enc-1': { pluginId: 'audio-encoder', displayName: 'Encoder', health: 'ok', running: true, ready: true, settings: { lcpSortOrder: 5 }, lcpType: 'mixer-strip' },
        });
    ...
    it('updateState merges with existing module', () => {
        const store = useModuleStore();
        store.setAll({
            'mic-1': { pluginId: 'audio-input', displayName: 'Mic 1', health: 'stopped', running: false, ready: false, settings: { lcpVisible: true }, lcpType: 'mixer-strip' },
        });
    ...
    it('applyConfig updates settings', () => {
        ...
        store.applyConfig({ 'mic-1': { displayName: 'Mic 1', settings: { volume: 75 }, lcpType: 'mixer-strip' } });
    ...
    it('applyConfig removes deleted modules', () => {
        const store = useModuleStore();
        store.setAll({
            'mic-1': { pluginId: 'audio-input', displayName: 'Mic 1', health: 'ok', running: true, ready: true, settings: {}, lcpType: 'mixer-strip' },
            'mic-2': { pluginId: 'audio-input', displayName: 'Mic 2', health: 'ok', running: true, ready: true, settings: {}, lcpType: 'mixer-strip' },
        });
        store.applyConfig({ 'mic-1': { displayName: 'Mic 1', settings: {}, lcpType: 'mixer-strip' } });
    ...
    it('applyPatch updates a single setting', () => {
        const store = useModuleStore();
        store.setAll({
            'mic-1': { pluginId: 'audio-input', displayName: 'Mic 1', health: 'ok', running: true, ready: true, settings: { volume: 100, audioEnabled: true }, lcpType: 'mixer-strip' },
        });
        store.applyPatch([{ op: 'replace', path: '/modules/mic-1/settings/audioEnabled', value: false }]);
    ...
    it('applyPatch with full config replace (path=/)', () => {
        ...
        store.applyPatch([{
            op: 'replace', path: '/',
            value: { modules: { 'new-1': { pluginId: 'audio-output', displayName: 'New 1', settings: { volume: 50 }, lcpType: 'mixer-strip' } } },
        }]);
