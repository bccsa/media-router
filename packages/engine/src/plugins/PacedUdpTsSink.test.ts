...
describe('packetizeTs', () => {
    it('emits whole TS datagrams and carries the remainder forward', () => {
    ...
    it('prepends leftover bytes to the next chunk', () => {
        const leftover = Buffer.alloc(50);
        const chunk = Buffer.alloc(TS_DATAGRAM_BYTES); // 50 + 188 = 238 → one datagram + 50 remainder
        ...
        expect(remainder.length).toBe(50);
    });

    it('buffers sub-datagram input without emitting', () => {
        const { datagrams, remainder } = packetizeTs(Buffer.alloc(0), Buffer.alloc(100));
