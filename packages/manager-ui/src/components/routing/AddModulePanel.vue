    ...
    <div class="fixed right-0 top-0 h-screen w-80 z-20 flex flex-col shadow-xl bg-card border-l border-border">
        ...
        <div class="flex-1 overflow-y-auto px-4 py-2">
            ...
            <template v-else-if="selectedPlugin">
                <button class="flex items-center gap-1 text-xs mb-3 text-accent-fg" @click="selectedPlugin = null">
                    ← Back
                </button>
                <div class="space-y-3">
                    <div class="flex items-center gap-2">
                        ...
                        <div>
                            <div class="text-sm font-medium text-foreground">{{ selectedPlugin.displayName }}</div>
                            <div class="text-xs mt-0.5 text-muted">{{ selectedPlugin.description }}</div>
                        </div>
                    </div>
                    <div v-if="selectedPlugin.ports.length > 0">
                        <div class="text-[10px] font-semibold uppercase tracking-wider mb-1.5 text-muted">Ports</div>
                        <div class="space-y-1">
                            <div v-for="port in selectedPlugin.ports" :key="port.id" class="flex items-center gap-2 text-xs text-subtle">
                                <div class="w-2 h-2 rounded-full" :style="{ backgroundColor: portColor(port.streamType) }" />
                                <span>{{ port.label || port.id }}</span>
                                <span class="text-[10px] px-1 rounded text-muted bg-border">{{ port.direction }}</span>
                            </div>
                        </div>
                    </div>
                    <div>
                        <label class="block text-xs font-medium mb-1 text-subtle">Display Name</label>
                        <input v-model="moduleName" type="text" class="w-full px-3 py-1.5 text-sm rounded-md bg-input border border-border text-foreground" />
                    </div>
                </div>
            </template>
            ...
            <template v-else>
                <div v-for="(items, category) in groupedPlugins" :key="category" class="mb-4">
                    <div class="text-[10px] font-semibold uppercase tracking-wider mb-1.5 px-1 text-muted">
                    ...
                    <button v-for="plugin in items" :key="plugin.pluginId" @click="selectPlugin(plugin)"
                        ...
                        <div>
                            <div class="font-medium text-xs text-foreground">{{ plugin.displayName }}</div>
                            <div class="text-[11px] mt-0.5 text-muted">{{ plugin.description }}</div>
                        </div>
                    </button>
                </div>
                <div v-if="Object.keys(groupedPlugins).length === 0" class="text-center py-8 text-sm text-muted">No plugins found.</div>
            </template>
        </div>

        <div v-if="selectedPlugin" class="px-4 py-3 flex justify-end gap-2 shrink-0 border-t border-border">
