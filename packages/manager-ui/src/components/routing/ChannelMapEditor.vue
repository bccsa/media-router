...
<template>
    ...
    >
        <div
            class="rounded-xl shadow-2xl overflow-hidden flex flex-col bg-card border border-border"
        ...
        >
            ...
            <!-- Source → Dest header -->
            <div
                class="flex items-center justify-between px-4 py-2 text-xs bg-surface-alt text-subtle"
            >
                <div class="flex items-center gap-1">
                    <div
                        class="w-2 h-2 rounded-full"
                        :style="{ backgroundColor: srcModule?.color ?? '#3b82f6' }"
                    />
                ...
                <div class="flex items-center gap-1">
                    <div
                        class="w-2 h-2 rounded-full"
                        :style="{ backgroundColor: dstModule?.color ?? '#10b981' }"
                    />
            ...
            <!-- Presets -->
            <div
                class="flex items-center gap-1 px-4 py-2 overflow-x-auto no-scrollbar border-b border-border-alt"
            >
            ...
            <div class="flex-1 overflow-auto px-4 py-3">
                ...
                >
                    ...
                    <line
                        ...
                        :key="`${m.srcChannel}-${m.dstChannel}-${i}`"
                        :x1="getSrcDotPos(m.srcChannel).x"
                        :y1="getSrcDotPos(m.srcChannel).y"
                        :x2="getDstDotPos(m.dstChannel).x"
                        :y2="getDstDotPos(m.dstChannel).y"
                        stroke="var(--accent)"
                        stroke-width="2"
                        stroke-opacity="0.7"
                    ...
                    <line
                        v-if="dragging"
                        :x1="getSrcDotPos(dragging.srcChannel).x"
                        :y1="getSrcDotPos(dragging.srcChannel).y"
                        :x2="dragging.x"
                        :y2="dragging.y"
                    ...
                    <g v-for="ch in srcChannels" :key="`src-${ch}`">
                        <circle
                            :cx="getSrcDotPos(ch - 1).x"
                            :cy="getSrcDotPos(ch - 1).y"
                            r="8"
                            fill="var(--bg-input)"
                            stroke="var(--accent)"
                            stroke-width="2"
                        ...
                        />
                        <text
                            :x="getSrcDotPos(ch - 1).x + 16"
                            :y="getSrcDotPos(ch - 1).y + 4"
                    ...
                    <g v-for="ch in dstChannels" :key="`dst-${ch}`">
                        <circle
                            :cx="getDstDotPos(ch - 1).x"
                            :cy="getDstDotPos(ch - 1).y"
                            r="8"
                        ...
                        />
                        <text
                            :x="getDstDotPos(ch - 1).x - 16"
                            :y="getDstDotPos(ch - 1).y + 4"
                            fill="var(--text-secondary)"
                            font-size="11"
                            text-anchor="end"
                        >
            ...
            <div
                ...
                class="px-4 pb-2 max-h-32 overflow-auto border-t border-border-alt"
            >
                ...
                <div
                    v-for="(m, i) in mappings"
                    :key="`map-${m.srcChannel}-${m.dstChannel}`"
                ...
                >
                    ...
                    <button
                        @click="removeMapping(i)"
                        class="p-0.5 rounded opacity-50 hover:opacity-100 text-error"
                    >
                        <svg
                            width="10"
                            height="10"
