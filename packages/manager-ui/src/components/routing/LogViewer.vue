...
<template>
    <div class="flex flex-col h-full">
        <!-- Toolbar -->
        <div
            class="flex items-center gap-2 px-3 py-1.5 shrink-0 overflow-x-auto border-b border-border bg-card"
        >
            <!-- Level filter -->
            ...
            <!-- Clear -->
            <button
                @click="logStore.clear(props.engineId)"
        ...
        <div
            ref="scrollEl"
            class="flex-1 overflow-y-auto font-mono text-[11px] leading-relaxed bg-surface"
            @scroll="
                () => {
                    if (scrollEl) {
                        const atBottom =
        ...
        >
            ...
            >
                ...
                <span class="shrink-0 w-10" :style="{ color: levelColor(entry.level) }">{{
                ...
                <span class="break-all whitespace-normal text-foreground">
                    ...
                    <template v-if="entry.instanceId">
                        <span class="text-muted">
                            [{{
                                getModuleName(entry.instanceId as string) || entry.instanceId
                            }}]</span
                        >
                    </template>
                </span>
