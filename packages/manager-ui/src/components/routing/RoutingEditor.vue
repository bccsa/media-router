...
const {
    ...
    onNodeDragStop,
    onEdgeDelete,
    onAddModule: graphAddModule,
    focusModule: graphFocusModule,
...
<template>
    <div ref="containerRef" class="w-full h-full relative flex flex-col">
        <!-- Toolbar -->
        <div
            class="absolute top-3 left-3 right-3 z-10 flex items-center gap-2 flex-wrap no-scrollbar toolbar-h"
        >
            <!-- Engine start/stop -->
            <MrTooltip
                :text="
                    engine?.running
                        ...
                        : 'Start all modules on this engine'
                "
            >
                <MrButton
                    v-if="engine?.running"
            ...
            <MrTooltip text="Stop all modules, restart PipeWire, and restart modules" width="w-52">
                <MrButton size="sm" variant="secondary" @click="showResetConfirm = true"
                    >Reset</MrButton
                >
            </MrTooltip>
            ...
            <MrTooltip
                ...
                width="w-44"
            >
                <MrButton size="sm" variant="secondary" @click="showLogs = !showLogs"
                    >Logs</MrButton
                >
            </MrTooltip>
            <MrTooltip
                :text="
                    focusMode
                        ? 'Exit focus mode — show all modules'
                        : 'Enter focus mode — dim non-focused modules'
                "
                width="w-52"
            >
                <MrButton
            ...
            <MrTooltip text="Click to list all modules — click a module to jump to it">
                ...
                <Teleport to="body">
                    <div
                        ...
                        @click="showModuleList = false"
                    />
                    <div
                        v-if="showModuleList"
                        class="fixed w-56 rounded-lg shadow-xl z-[999] flex flex-col bg-card border border-border"
                        :style="{
                            top: moduleListPos.y + 'px',
                            left: moduleListPos.x + 'px',
                            maxHeight: '280px',
                        }"
                    >
                        ...
                        <div class="overflow-y-auto py-1">
                            ...
                            >
                                ...
                                />
                                {{ mod.displayName }}
                                <span class="text-[10px] ml-auto text-muted">{{
                                    mod.pluginId
                                }}</span>
                            </button>
                        </div>
        ...
        >
            <template #node-module="{ data, id }">
                <ModuleNode
                    :data="data"
                    @dblclick="settingsPanel = { moduleId: id }"
        ...
        <MrContextMenu
            ...
            @slider-change="onContextSliderChange"
            @toggle-change="onContextToggleChange"
        ...
        <!-- Edge label editor -->
        <EdgeLabelEditor
            v-if="editingEdgeLabel"
            :label="editingEdgeLabel.label"
            @save="
                (l: string) => {
                    editingEdgeLabel!.label = l;
                    saveEdgeLabel();
                }
            "
        ...
        <Teleport to="body">
            ...
            >
                <div
                    class="rounded-lg shadow-xl p-5 max-w-sm w-full mx-4 bg-card border border-border"
                >
                    <h3 class="text-base font-semibold mb-2 text-foreground">Reset Engine?</h3>
                    <p class="text-sm mb-4 text-subtle">
                        This will stop all modules, restart PipeWire, and restart all modules. Audio
                        will be interrupted for a few seconds.
                    </p>
                    <div class="flex justify-end gap-2">
                        <MrButton size="sm" variant="secondary" @click="showResetConfirm = false"
                            >Cancel</MrButton
                        >
