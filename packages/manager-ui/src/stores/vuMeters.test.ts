...
describe('useVuStore', () => {
    ...
    it('holds the last value until the 2.5s stale threshold elapses', async () => {
        vi.useFakeTimers();
        const store = useVuStore();
        store.update('eng-1', 'mod-1', [8, 8]);

        // Just before the 2.5s threshold — should still hold the last value
        vi.advanceTimersByTime(2400);
        expect(store.get('eng-1', 'mod-1')).toEqual([8, 8]);

        // Past 2.5s threshold + one cleanup tick (500ms) — should be zeroed
