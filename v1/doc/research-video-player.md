...
GST_DEBUG=2,pulsesink:6 gst-launch-1.0 srtserversrc name=srtserversrc uri="srt://10.9.1.51:2000?mode=caller&latency=10" wait-for-connection=false ! tsparse ignore-pcr=true ! tsdemux ignore-pcr=true latency=1 name=t t. ! h264parse ! v4l2h264dec ! queue flush-on-eos=true leaky=2 max-size-time=50000000 ! kmssink sync=false async=false t. ! aacparse ! avdec_aac ! audioconvert ! audiorate tolerance=40000000 skip-to-first=true ! queue flush-on-eos=true leaky=2 max-size-time=50000000 ! pulsesink device=MR_PA_SrtVideoPlayer_1031 sync=false slave-method=1 processing-deadline=40000000
```